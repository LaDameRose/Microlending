...
```{r}
mlr_full <- lm(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + MPI + country, data = comp)
...
```{r}
reg_mlr <- lm(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp)
```
...
comp_norm <- cbind(as.data.frame(lapply(comp[,c(2,4,7,8)], normalize)), country = comp$country, sector = comp$sector, borrower_genders = comp$borrower_genders, repayment_interval = comp$repayment_interval, funded_month = comp$funded_month, posted_month = comp$posted_month)
norm_mlr <- lm(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_norm)
summary(norm_mlr)
```

Log-log transform and Multiple Linear Regression
```{r}
comp_log <- cbind(as.data.frame(log(comp[,c(2,4,7,8)])), country = comp$country, sector = comp$sector, borrower_genders = comp$borrower_genders, repayment_interval = comp$repayment_interval, funded_month = comp$funded_month, posted_month = comp$posted_month)
log_mlr <- lm(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_log)
summary(log_mlr)
```
...
reg_mlr_cv <- train(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_train, method = "lm", trControl = comp_train_control)
reg_mlr_cv$resample
reg_mlr_cv$results
```
...
```{r}
reg_mlr_model <- lm(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_train)
...
```{r}
norm_mlr_cv <- train(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_norm_train, method = "lm", trControl = comp_train_control)
...
```{r}
norm_mlr_model <- lm(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_norm_train)
...
```{r}
log_mlr_cv <- train(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_log_train, method = "lm", trControl = comp_train_control)
log_mlr_cv$resample
log_mlr_cv$results
```
...
```{r}
log_mlr_model <- lm(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_log_train)
...
```{r}
reg_rt_cv <- train(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_train, method = "rpart", trControl = comp_train_control)
reg_rt_cv$resample
reg_rt_cv$results
```
...
```{r}
reg_rt_model <- rpart(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_train, method = "anova")
...
```{r}
norm_rt_cv <- train(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_norm_train, method = "rpart", trControl = comp_train_control)
norm_rt_cv$resample
norm_rt_cv$results
```
...
```{r}
norm_rt_model <- rpart(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_norm_train, method = "anova")
...
```{r}
log_rt_cv <- train(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_log_train, method = "rpart", trControl = comp_train_control)
log_rt_cv$resample
log_rt_cv$results
```
...
```{r}
log_rt_model <- rpart(duration_perdollar ~ funded_amount + sector + term_in_months + borrower_genders + repayment_interval + funded_month + posted_month + country, data = comp_log_train, method = "anova")
...
```{r}
reg_rf_model <- randomForest(comp_train[,-c(1,7)], comp_train$duration_perdollar, ntree = 50, do.trace = TRUE)
...
```{r}
norm_rf_model <- randomForest(comp_norm_train[,-c(3,5)], comp_norm_train$duration_perdollar, ntree = 50, do.trace = TRUE)
...
```{r}
log_rf_model <- randomForest(comp_log_train[,-c(3,5)], comp_log_train$duration_perdollar, ntree = 50, do.trace = TRUE)
