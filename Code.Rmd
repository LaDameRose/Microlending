...
```{r}
comp_log <- cbind(as.data.frame(log(comp[,c(2,4,7,8)])), sector = comp$sector, borrower_genders = comp$borrower_genders, repayment_interval = comp$repayment_interval, posted_month = comp$posted_month)
...
```{r}
norm_rf_model <- randomForest(comp_norm_train[,-c(3,5,9)], comp_norm_train$duration_perdollar, ntree = 50, do.trace = TRUE)
...
```{r}
log_rf_model <- randomForest(comp_log_train[,-c(3,5,9)], comp_log_train$duration_perdollar, ntree = 50, do.trace = TRUE)
